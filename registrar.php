...
$res= mysqli_query($conexion,"SELECT * FROM usuarios JOIN estacionamientos ON usuarios.id=
estacionamientos.id JOIN autos ON estacionamientos.id =
autos.id WHERE usuarios.is_deleted = '0'");
?>
...
<div class="card-deck">
    ...
    <br>
        ...
        <table class="table table-responsive">
            ...
            <tbody>
                <?php  
                $sql = "SELECT id,fecha,ciudad,lugar,posicion FROM estacionamientos WHERE is_deleted = 0 ";
                $resultado = mysqli_query($conexion,$sql);
                while($row = mysqli_fetch_assoc($resultado)){
                ?>
                <tr>
