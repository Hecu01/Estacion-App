...
if(isset($_POST['username']) && isset($_POST['contraseña'])) {
    ...
    if($row = mysqli_fetch_assoc($res)){
        if($row['rol_id'] == 1){
            ...
            if($row['rol_id'] == 2){
          echo '<div class= "alerta">El usuario no es válido';
        }
    }
     }
...
if(isset($row) && isset($res)){
    if($row = mysqli_fetch_assoc($res)){
    if($row['rol_id'] == 2){
        echo '<div class= "alert alert-danger">El usuario no es válido</div>';
    }
}
}
...
<h4 align="center"><strong>Nombre de Usuario</strong></h4>
<br>
